...
class KafkaIntegrationTest(TransactionTestCase):
    ...
    def test_node_creation_emits_event_and_audit_consumer_writes_log(self):
        """
        ...
        # We need to patch the 'producer' attribute of the global 'producer' INSTANCE in events.producer
        from events.producer import producer as global_producer
        
        # Create a mock for the confluent_kafka.Producer
        mock_kafka_producer = MagicMock()
        
        # Patch the instance attribute '_producer' which stands behind the property
        with patch.object(global_producer, '_producer', mock_kafka_producer):
            ...
            # EventLog.payload stores the FULL event body, including metadata
            self.assertEqual(log_entry.payload['payload']['name'], "Integration-Test-Node")
